...
extern GLuint glTextureLoadPVR(
  char *fname,
...
static void drawGl(float rot, float x, float y) {
...
static void drawOrthoScene(void) {
  ...
  // Draw a nice 32x32 bitsize square checkered background
  glBindTexture(GL_TEXTURE_2D, texture);
  rect(0, 0, 512, 512);
  rect(512, 0, 512, 512);
}

static void drawPerspectiveScene(void) {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
